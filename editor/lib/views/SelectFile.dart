...
class SelectFileWidget extends StatelessWidget {
  ...
  const SelectFileWidget({super.key, this.onFileSelected});

  @override
  Widget build(BuildContext context) {
    return SingleChildScrollView(
      child: DirectoryTreeViewer(
        rootPath: join(Directory.current.path, 'assetdata'),
        onFileTap: (file, tap) {
          onFileSelected?.call(file);
        },
        fileActions: [IconButton(onPressed: () {}, icon: Icon(Icons.delete))],

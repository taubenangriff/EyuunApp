...
class PickUpbringingController {
  ...
  List<PersonSize> getPossibleSizes() {
    var fromUpbringing =
        selectedUpbringing?.get<UpbringingComponent>()?.possibleSizes ?? [];
    var fromAdditional =
        selectedAdditionalUpbringing
            ?.get<UpbringingComponent>()
            ?.possibleSizes ??
        [];
  ...
  List<Entity> getCurrentlySelectableUpbringings() {
    return locator<CharacterTablesFeatureComponent>().upbringings
        .where((e) => e != selectedAdditionalUpbringing)
        .toList();
  }

  List<Entity> getCurrentlySelectableAdditionalUpbringings() {
    return locator<CharacterTablesFeatureComponent>().secondaryUpbringings
        .where((e) => e != selectedUpbringing)
        .toList();
  }
  ...
  bool pickedBoth() =>
      selectedAdditionalUpbringing != null && selectedUpbringing != null;
  ...
  setBuffProvidingUpbringing(Entity e) {
    if (e != selectedUpbringing && e != selectedAdditionalUpbringing) {
      throw ArgumentError(
        "Buff providing upbringing must be either one of upbringing or additional upbringing!",
      );
    }
    _buffProvidingUpbringing = e;
  }

  void finalizeUpbringings() {
    ...
    var visualUpbringing = buffProvidingUpbringing == selectedUpbringing
        ? selectedAdditionalUpbringing
        : selectedUpbringing;
